...
//-----------------------------------------------------------------------
case class RecoverLeaf(newlist: ArrayBuffer[Int], theDead: Int)
case class RecoverRoutingTable(row: Int, column: Int, newID: Int)
case class RemoveNode(removeID : Int)
case class RequestInRoutingTable(row: Int, column: Int)
case class RequestLeafWithout(theID: Int)
case object FailNodes
case object NodeDie
//-----------------------------------------------------------------------
...
object Project3bonus extends App{
	/*
		...
		*	numGailingNodes are the total number of nodes failing
	*/
	var numNodes = 0
	var numRequests = 0
	var numFailingNodes = 0
	if(args.length != 3){
		println("************************Invalid Input**********************\n" + 
				"Input should be of type : \n scala Project3 numNodes numRequests numFailingNodes\n" +
				"***********************************************************")
		System.exit(1)
	}
	else{
			...
			numFailingNodes = args(2).toInt
			
			if(numFailingNodes >= numNodes){
				println(" More number of failing nodes than total number of nodes ")
				System.exit(1)
			}
			else{
				if(numFailingNodes < 0){
					println("Not a valid number of nodes ( Cannot be negative ) ")
					System.exit(1)
				}
				else{
						println("Num Nodes : " + numNodes + " Num Requests : " + numRequests + " Num Failing Nodes " + numFailingNodes);
						pastry_protocol( numNodes , numRequests , numFailingNodes)
				}
			}	
	}

	def pastry_protocol(numNodes : Int , numRequests : Int , numFailingNodes : Int){
		println("Reached pastry_protocol")
		val system = ActorSystem("Project3")
      	val master = system.actorOf(Props(new Master(numNodes, numRequests , numFailingNodes)), name = "master")
      	master ! Start
	}

}

class Master (numNodes : Int , numRequests : Int , numFailingNodes : Int) extends Actor{
	/*
	...
	*/
	...
	def receive = {
	    ...
	    case RouteFinish(requestFrom, requestTo, hops) =>
	      ...
	      numHops += hops
	      for (i <- 1 to 10)
	        if (numRouted == (numNodes - numFailingNodes) * numRequests * i / 10){
	          for ( j <- 1 to i)
	          	print(".")
	          print("|")
	        }
	      if (numRouted >= (numNodes - numFailingNodes) * numRequests) {
	      	println("\n")
	    ...
	    case FailNodes =>
		      for (i <- 0 until numFailingNodes)
		        context.system.actorSelection("/user/master/" + randomList(i)) ! NodeDie
		  
		  context.system.scheduler.scheduleOnce(1000 milliseconds, self, StartRouting)
...
class PastryNode(numNodes : Int , numRequests : Int , myID : Int , base : Int) extends Actor{ 
    ...
    def receive ={
			...
			case Route(msg, requestFrom, requestTo, hops) =>
									      ...
									      if (msg == "Join") {
									        ...
									        */
									        if ((smallerLeaf.length > 0 && requestTo >= smallerLeaf.min && requestTo <= myID) || //In less leaf set
									          ...
									          var diff = IDSpace + 10
									          var nearest = -1
									          if (requestTo < myID) { //In less leaf set
									            for (i <- smallerLeaf) {
									         ...
									         */
									          ...
									          var nearest = -1
									          for (i <- 0 until 4) {
									            if ((routingTable(samePrefix)(i) != -1) && (abs(routingTable(samePrefix)(i) - requestTo) < diff)) {
									              diff = abs(routingTable(samePrefix)(i) - requestTo)
									              nearest = routingTable(samePrefix)(i)
									            }
									          }
									          if (nearest != -1) {
									            if (nearest == myID) {
									              if (requestTo > myID) { //Not in both
									                context.system.actorSelection("/user/master/" + largerLeaf.max) ! Route(msg, requestFrom, requestTo, hops + 1)
				...
				case NodeDie =>
												context.system.actorSelection("/user/master/*") ! RemoveNode(myID)
												println(" Dead : " + myID)
												context.stop(self)

				case RemoveNode (removeID) =>
												//println("I'm being removed " + removeID)

												if (removeID > myID && largerLeaf.contains(removeID)) { //In larger leaf
											        largerLeaf -= removeID
											        if (largerLeaf.length > 0)
											          context.system.actorSelection("/user/master/" + largerLeaf.max) ! RequestLeafWithout(removeID)
											      }
											      if (removeID < myID && smallerLeaf.contains(removeID)) { //In less leaf
											        smallerLeaf -= removeID
											        if (smallerLeaf.length > 0)
											          context.system.actorSelection("/user/master/" + smallerLeaf.min) ! RequestLeafWithout(removeID)
											      }
											      var samePrefix = checkPrefix(toBase4String(myID, base), toBase4String(removeID, base))
											      if (routingTable(samePrefix)(toBase4String(removeID, base).charAt(samePrefix).toString.toInt) == removeID) {
											        routingTable(samePrefix)(toBase4String(removeID, base).charAt(samePrefix).toString.toInt) = -1
											        for (i <- 0 until 4) {
											          if (routingTable(samePrefix)(i) != myID && routingTable(samePrefix)(i) != removeID && routingTable(samePrefix)(i) != -1) {
											            context.system.actorSelection("/user/master/" + routingTable(samePrefix)(i)) !
											              RequestInRoutingTable(samePrefix, toBase4String(removeID, base).charAt(samePrefix).toString.toInt)
											          }
											        }
											      }
				case RequestLeafWithout(theID) =>
											      var temp = new ArrayBuffer[Int]()
											      temp ++= smallerLeaf ++= largerLeaf -= theID
											      sender ! RecoverLeaf(temp.clone, theID)

				case RecoverLeaf(newlist, theDead) =>
											      
											      for (i <- newlist) {
											      	//Can be added to largerLeaf
											        if (i > myID && !largerLeaf.contains(i)) { 
											          if (largerLeaf.length < 4) {
											            largerLeaf += i
											          } else {
				...
				case RequestInRoutingTable(samePrefix, column) =>
											      if (routingTable(samePrefix)(column) != -1)
											        sender ! RecoverRoutingTable(samePrefix, column, routingTable(samePrefix)(column))

				case RecoverRoutingTable(row, column, newID) =>
											      if (routingTable(row)(column) == -1) {
